...
class MineTopkDropoutStrategy(BaseSignalStrategy):
    # TODO:
    # 1. Supporting leverage the get_range_limit result from the decision
    # 2. Supporting alter_outer_trade_decision
    # 3. Supporting checking the availability of trade decision
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.only_tradable = only_tradable
        self.forbid_all_trade_at_limit = forbid_all_trade_at_limit
        self.buy_score_thresh = buy_score_thresh
        self.sell_score_thresh = sell_score_thresh
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        if self.method_buy == "top":
            # 加分数阈值过滤在不在last之外的股票
            can_buy_mask = (~pred_score.index.isin(last)) & (pred_score >= self.buy_score_thresh)
            candidates = pred_score[can_buy_mask]
            today = get_first_n(
        ...
        # Get the stock list we really want to sell (After filtering the case that we sell high and buy low)
        if self.method_sell == "bottom":
            sell_bottom = last[last.isin(get_last_n(comb, self.n_drop))]
            force_sell = last[pred_score.reindex(last) < self.sell_score_thresh]
            sell = pd.Index(sell_bottom).union(pd.Index(force_sell))
        elif self.method_sell == "random":
            candi = filter_stock(last)
            try:
                sell = pd.Index(np.random.choice(candi, self.n_drop, replace=False) if len(last) else [])
